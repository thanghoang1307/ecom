...
<body>
					...
					<div class="col-md-12">
						<ul class="nav justify-content-end header-user-list">
							...
							<li class="nav-item">@auth('customer')
								<a class="nav-link user-action" href="{{route('front.account.index')}}">{{Auth::guard('customer')->user()->name}} <i class="icon icon-user-head"></i></a>
								@endauth
								@guest('customer')
								<a class="nav-link user-action" data-toggle="modal" data-target="#loginModal">Đăng nhập/ Đăng ký <i class="icon icon-user-profile"></i></a>
								@endguest</li>
						</ul>
					</div>
				</div>
			...
			<div class="container">
				<div class="row align-items-center">
					...
					<div class="col-11 col-md-5">
						<!-- Start Side Menu -->
						<a class="menu-toggle collapsed fix-icon" data-toggle="collapse" href="#collapseMenu" role="button" aria-expanded="false" aria-controls="collapseMenu">
					...
					<div class="col-1 col-md-3">
						<div class="text-right">
							<!--USER ACTION-->
							@auth('customer')
							<a class="user-cart d-none d-md-inline-block" href="{{route('front.account.index')}}"><span>{{Auth::guard('customer')->user()->name}} <i class="icon icon-user-head"></i></span></a>
							@endauth
							@guest('customer')
							<a class="user-cart d-none d-md-inline-block" data-toggle="modal" data-target="#loginModal"><span>Đăng nhập/Đăng ký <i class="icon icon-user-head"></i></span></a>
							@endguest
							...
							<div id="cart" class="dropdown dropdown-discount">@if(session()->get('cart.items'))
								...
								@if(session()->get('cart.items'))
								<a class="highlight d-none d-md-block" href="{{route('front.check_out_1')}}" id="dropdownMenuButton" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
									<span class="header-cart-number">{{array_sum(session()->get('cart.items'))}}</span>
									<i class="icon icon-shopping-cart"></i>
								</a>
								@else
								<a class="highlight d-none d-md-block" href="#" id="dropdownMenuButton" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
									<i class="icon icon-shopping-cart"></i>
								</a>
								@endif
								...
								<div class=" d-none d-md-block">
									<div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
										...
										<div class="product-end">
											<h5 class="product-end-block"><span class="product-end-title">Thành tiền</span> <span class="product-end-block-price price">{{$cart_total}}<sup>đ</sup></span></h5>
	...
	<div class="collapse hidden-mobile" id="collapseMenu">
		<div class="hidden-bg menu-toggle" data-toggle="collapse" href="#collapseMenu" role="button" aria-expanded="false" aria-controls="collapseMenu">&nbsp;</div>
