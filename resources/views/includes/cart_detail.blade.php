					...
					<div class="row">
						...
						<div class="col-md-4">
							<div class="cart-price">
								<div class="text-right"><span class="active-price price">{{$prd->current_price}}<sup>đ</sup></span></div>
			                    @if($prd->sale_price)
								
								<div class="text-right"><span class="pre-active-price price">{{$prd->regular_price}}<sup>đ</sup></span></div>
			                    @endif
							</div>
						</div>
					</div>
		...
		@if(session()->get('cart.items'))
			...
			<div class="row align-items-center align-content-center">
				...
				<div class="col-6">
					<div class="cart-price">
						<div class="cart-price">
							<div class="text-right"><span class="last-price price">{{$total}}<sup>đ</sup></span></div>
						</div>
					</div>
				</div>
			</div>
	        ...
	        <!-- Proposal -->
	          <div class="cart-item">
				<div class="row no-gutters align-items-center align-content-center">
					<div class="col-4 col-md-12"><a href="{{route('front.tai_bao_gia')}}" target="_blank" class="btn-submit is-white bao-gia">
		                  <span class="d-none d-md-block">Tải về file báo giá (.pdf)</span>
		                  <span class="d-block d-md-none">Tải báo giá</span>
		                </a></div>
					
					<div class="col-8">
						<form action="@yield('next')" method="POST">@csrf
							<button type="submit" class="btn-submit d-block d-md-none"><span>Tiến hành thanh toán</span></button>
						</form>
					</div>
				</div>
			</div>
