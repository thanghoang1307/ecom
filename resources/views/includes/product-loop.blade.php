@foreach ($prds->take(10) as $prd)
	@if($prd->is_show)
        <div class="item"><a href="{{route('front.product-detail',$prd->slug)}}">
	<div class="product-item"><a href="{{route('front.product-detail',$prd->slug)}}">
		...
		<div class="product-hover {{$prd->short_desc ? '' : 'no-discount'}}">
			<div class="product-promotion"><i class="sale-label"></i> <span>{!!strip_tags($prd->short_desc)!!}</span></div>
		</div>
	</div>
</a></div>
	@endif
